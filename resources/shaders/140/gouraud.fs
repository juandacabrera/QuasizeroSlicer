...
void main()
{
    ...
    if (is_outline) {
        ...
        out_color = qz_finish(vec4(mix(color.rgb, getBackfaceColor(color.rgb), s), color.a));
    }
#ifdef ENABLE_ENVIRONMENT_MAP
    else if (use_environment_tex)
        out_color = qz_finish(vec4(0.45 * texture(environment_tex, normalize(eye_normal).xy * 0.5 + 0.5).xyz + 0.8 * color.rgb * intensity.x, color.a));
#endif
    else
        out_color = qz_finish(vec4(vec3(intensity.y) + color.rgb * intensity.x, color.a));
}