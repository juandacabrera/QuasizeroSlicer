...
- **After the collapse — the bead keeps coming** (`QzBeadSim`, `engine/src/QzBeadSim.{hpp,cpp}`,
  `engine/tests/test_qz_beadsim.cpp`). The head follows its nominal path; the bead
...
- **Rendering** (`QzSkeleton`, `engine/src/QzSkeleton.{hpp,cpp}`, `engine/tests/test_qz_skeleton.cpp`).
...
- **Player.** With the deformation view on, the moves slider runs in slow motion (1/10 of
  the layer pace, about a third of real time) from 0.5 s before t_collapse to 3.5 s after
  it, then resumes. During the layer change of the play the visible range points at the end
  of the new layer for a frame or two; the deformed view keeps its last mesh through that
  transient (otherwise the collapsed state flashes a layer early).
