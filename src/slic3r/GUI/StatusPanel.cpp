...
static const wxColour BUTTON_HOVER_COL   = wxColour(58, 56, 53);

static const wxColour DISCONNECT_TEXT_COL = wxColour(171, 172, 172);
static const wxColour NORMAL_TEXT_COL     = wxColour(60, 54, 49);
...
static const wxColour STAGE_TEXT_COL      = wxColour(58, 56, 53);
...
void ExtruderSwithingStatus::updateBy(const DevExtderSystem* ext_system)
{
    ...
    auto state = ext_system->GetSwitchState();
    {
        if (state == DevExtderSwitchState::ES_SWITCHING)
        {
            ...
            m_switching_status_label->SetForegroundColour(StateColor::darkModeColorFor("#30302F"));
...
void PrintingTaskPanel::create_panel(wxWindow* parent)
{
    ...
    m_staticText_progress_percent->SetForegroundColour(wxColour(58, 56, 53));
    ...
    m_staticText_progress_percent_icon->SetForegroundColour(wxColour(58, 56, 53));
...
wxBoxSizer *StatusBasePanel::create_misc_control(wxWindow *parent)
{
    ...
    m_switch_fan->Bind(wxEVT_ENTER_WINDOW, [this](auto& e) {
        m_fan_panel->SetBackgroundColor(wxColour(58, 56, 53));
        });
...
wxBoxSizer* StatusBasePanel::create_filament_group(wxWindow* parent)
{
    ...
    m_title_filament_loading->SetForegroundColour(wxColour(45, 44, 42));
...
void StatusPanel::update(MachineObject *obj)
{
    ...
    if (m_qz_syringe) {
        ...
        if (qz_on) {
            ...
            // per-update: auto-unwind the live Z babystep only when the job truly
            // ends - a refill/user PAUSE keeps the offset so resume lands right
            m_qz_syringe->update_print_state(obj != nullptr && (obj->is_in_printing() || obj->is_in_printing_pause()));
