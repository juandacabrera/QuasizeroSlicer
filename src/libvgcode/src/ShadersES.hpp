...
"out vec3 color;\n"
"out vec3 qz_eye_normal;\n"
"out vec3 qz_eye_pos;\n"
"out vec3 qz_world_pos;\n"
"out vec3 qz_world_normal;\n"
...
"  color = color_base;\n"
"  qz_eye_normal = eye_normal;\n"
"  qz_eye_pos = eye_position;\n"
"  qz_world_pos = pos;\n"
"  qz_world_normal = normalize(pos - endpoint_pos);\n"
...
"in vec3 color;\n"
"in vec3 qz_eye_normal;\n"
"in vec3 qz_eye_pos;\n"
"in vec3 qz_world_pos;\n"
"in vec3 qz_world_normal;\n"
"out vec4 fragment_color;\n"
"const vec3 L1 = vec3(-0.4574957, 0.4574957, 0.7624929);\n"
"const vec3 L2 = vec3(0.6985074, 0.1397015, 0.6985074);\n"
"float qz_hash(vec3 p) { p = fract(p * 0.3183099 + 0.1); p *= 17.0; return fract(p.x * p.y * p.z * (p.x + p.y + p.z)); }\n"
"float qz_noise(vec3 p) {\n"
"  vec3 i = floor(p); vec3 f = fract(p); f = f * f * (3.0 - 2.0 * f);\n"
"  return mix(mix(mix(qz_hash(i), qz_hash(i + vec3(1.0,0.0,0.0)), f.x),\n"
"                 mix(qz_hash(i + vec3(0.0,1.0,0.0)), qz_hash(i + vec3(1.0,1.0,0.0)), f.x), f.y),\n"
"             mix(mix(qz_hash(i + vec3(0.0,0.0,1.0)), qz_hash(i + vec3(1.0,0.0,1.0)), f.x),\n"
"                 mix(qz_hash(i + vec3(0.0,1.0,1.0)), qz_hash(i + vec3(1.0,1.0,1.0)), f.x), f.y), f.z);\n"
"}\n"
"vec3 qz_aces(vec3 x) { return clamp((x * (2.51 * x + 0.03)) / (x * (2.43 * x + 0.59) + 0.14), 0.0, 1.0); }\n"
"void main() {\n"
"  vec3 n = normalize(qz_eye_normal);\n"
"  vec3 v = normalize(-qz_eye_pos);\n"
"  vec3 base = pow(max(color, vec3(0.0)), vec3(2.2));\n"
"  float g = 0.82 + 0.28 * qz_noise(qz_world_pos * 14.0);\n"
"  g *= 0.92 + 0.12 * qz_noise(qz_world_pos * 3.5);\n"
"  base *= g;\n"
"  float topness = clamp(qz_world_normal.z * 0.5 + 0.5, 0.0, 1.0);\n"
"  float ao = mix(0.55, 1.0, pow(topness, 0.6));\n"
"  float d1 = max(dot(n, L1), 0.0);\n"
"  float d2 = max(dot(n, L2), 0.0);\n"
"  vec3 h1 = normalize(L1 + v);\n"
"  float spec = pow(max(dot(n, h1), 0.0), 24.0) * 0.18;\n"
"  vec3 lin = base * (0.34 + 0.72 * d1 + 0.30 * d2) * ao + vec3(spec) * ao;\n"
"  fragment_color = vec4(pow(qz_aces(lin * 1.25), vec3(1.0 / 2.2)), 1.0);\n"
"}\n";
