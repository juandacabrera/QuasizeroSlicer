...
// nodes), and QzTubeMesher: a continuous tube re-skin of a displaced skeleton.
// GNU AGPLv3 inside the public Quasizero Slicer LITE (it draws what the engine computes);
// the Quasizero simulation engine carries an identical copy (dual-licensed, see LICENSE).
//
...
// Include guard rather than `#pragma once`: a PRO build of the slicer reaches this header
// and the engine's identical copy through different paths (the build checks that they match).
#ifndef QZ_SKELETON_HPP
#define QZ_SKELETON_HPP

#include "QzSimTypes.hpp"
...
{
public:
    // Segments must be in deposition order (as the simulator stores them). Two consecutive
